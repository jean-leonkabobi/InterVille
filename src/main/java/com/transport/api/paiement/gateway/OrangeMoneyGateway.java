...
public class OrangeMoneyGateway implements PaymentGateway {
    ...
    private static final Logger logger = LoggerFactory.getLogger(OrangeMoneyGateway.class);
    private final ConversionService conversionService;

    @Override
    public PaiementResponse processPayment(PaiementRequest request) {
        logger.info("Traitement paiement Orange Money pour réservation {} au {}",
                request.getReservationId(), request.getPhoneNumber());

        String transactionRef = "OM" + UUID.randomUUID().toString().substring(0, 8).toUpperCase();
    ...
    public boolean supports(ModePaiement mode) {
        return mode == ModePaiement.ORANGE_MONEY;
    }
}